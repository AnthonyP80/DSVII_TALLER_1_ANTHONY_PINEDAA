...
function contarCiudadesPorInicial($arr, $letra) {
    ...
    foreach ($arr as $ciudad) {
        // Compara la primera letra (en mayúsculas) con la letra indicada (también en mayúsculas)
        if (strtoupper(substr($ciudad, 0, 1)) === strtoupper($letra)) {
            $contador++;
        }
    }
    return $contador;
...
// Ejemplo de uso:
$letraInicial = 'S';
echo "\nNúmero de ciudades que comienzan con la letra '$letraInicial': "
     . contarCiudadesPorInicial($ciudades, $letraInicial) . "\n";
